#!/bin/bash
# Git Hooks 自动化代码审查 - 安装脚本
# 使用方法：在 Git 仓库根目录执行此脚本
...
for h in pre-commit prepare-commit-msg commit-msg pre-push post-merge post-checkout; do
  if [ -f "$ROOT/.git/hooks/$h" ]; then
    cp "$ROOT/.git/hooks/$h" "$ROOT/.git/hooks/$h.bak.$(date +%s)"
    echo "已备份旧 $h"
  fi
done

# 创建配置文件
cat > "$ROOT/.claude-git" << 'CFG'
...
if [ -s "$DIFF_FILE" ]; then
    source "$ROOT/.claude-git" 2>/dev/null
    echo "🤖 Claude 正在审查..."
    RESULT=$(claude -p "${STAGED_PROMPT}。请读取文件 $DIFF_FILE（git diff 格式），按上述要求回答。" --output-format text 2>&1)
...
if [ -s "$DIFF_FILE" ]; then
    RESULT=$(claude -p "${COMMIT_PROMPT}。请读取文件 $DIFF_FILE（git diff 格式），只返回一条符合 Conventional Commits 规范的中文 commit message，不要解释。格式示例：feat: 新增用户登录功能" --output-format text 2>&1)
...
while read LOCAL_REF LOCAL_SHA REMOTE_REF REMOTE_SHA; do
    ...
    if [ "$REMOTE_SHA" = "0000000000000000000000000000000000000000" ]; then
        git log --patch "$LOCAL_SHA" -1 > "$DIFF_FILE"
    else
        git log --patch "$REMOTE_SHA..$LOCAL_SHA" > "$DIFF_FILE"
    fi
    if [ -s "$DIFF_FILE" ]; then
        ROOT=$(git rev-parse --show-toplevel)
        source "$ROOT/.claude-git" 2>/dev/null
        echo "🤖 Claude 正在审查即将推送的代码..."
        RESULT=$(claude -p "${PUSH_PROMPT}。请读取文件 $DIFF_FILE（git diff 格式），按上述要求回答。" --output-format text 2>&1)
        TIMESTAMP=$(date +%Y%m%d%H%M%S)
...
(
    ...
    source "$ROOT/.claude-git" 2>/dev/null
    DIFF_FILE=$(mktemp)
    git diff ORIG_HEAD..HEAD > "$DIFF_FILE"
    if [ -s "$DIFF_FILE" ]; then
        RESULT=$(claude -p "${MERGE_PROMPT}。请读取文件 $DIFF_FILE（git diff 格式），按上述要求回答。" --output-format text 2>&1)
...
if git reflog -1 | grep -qE 'pull|merge'; then
    (
        ...
        source "$ROOT/.claude-git" 2>/dev/null
        DIFF_FILE=$(mktemp)
        git diff "$PREV_HEAD..$NEW_HEAD" > "$DIFF_FILE"
        if [ -s "$DIFF_FILE" ]; then
            RESULT=$(claude -p "${PULL_PROMPT}。请读取文件 $DIFF_FILE（git diff 格式），按上述要求回答。" --output-format text 2>&1)
            COMMIT_ID=$(git rev-parse --short HEAD)
            TMPFILE="/tmp/claude-review-${COMMIT_ID}.md"
            {
